...
| `.claude/hooks/agent-notify.sh` | byte-identical | — | Stop/Notification → desktop toast (`terminal-notifier`/`osascript` Mac, `notify-send` Linux); no-ops where no notifier exists. Wired in `.claude/settings.json`. Un-ignored via a `!.claude/hooks/agent-notify.sh` negation in `.gitignore` — **sibling `.gitignore`s need the same negation** to track the synced copy. |
