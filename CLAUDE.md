...
| `.claude/hooks/agent-status.sh` | byte-identical | — | Stop/Notification → writes `/tmp/agent-status-<uid>-<pane>` (Stop → `read` if the pane is visible else `unread`; Notification → `waiting`), then nudges `tmux-agents.sh refresh`. Read by the agent sidebar to color the tab/agent dot (working=title spinner, so not written here). **Not** herdr-gated (the sidebar needs status under herdr too). Wired in `.claude/settings.json` alongside agent-notify. Un-ignored via a `!.claude/hooks/agent-status.sh` negation in mac `.gitignore` (dots-linux doesn't ignore `.claude`, so no negation needed there). |
